<template>
  <div v-if="modelValue" class="sg-backdrop" @click.self="close">
    <div class="sg-card">
      <button class="btn-close sg-close" @click="close" aria-label="Close"></button>

      <div class="sg-header">
        <h5 class="m-0">스티커 남기기</h5>
        <div class="small text-muted">
          완료된 1:1 산책의 상대에게 스티커를 보내세요. 스티커를 누르면 즉시 전송됩니다.
        </div>
      </div>
      ...
      <div class="sg-body">
        ...
        <div class="d-flex align-items-center gap-3 mb-3">
          <img :src="partnerMeta.avatar || FALLBACK_AVATAR" class="sg-avatar" alt="상대 프로필" />
          <div>
            <div class="fw-bold">
              {{ partnerMeta.petName || partnerName || 'USER' }}
              <small class="text-muted">(#{{ partnerId }})</small>
            </div>
            <div class="small text-muted">요청ID: {{ requestOneId ? requestOneId : '-' }}</div>
          </div>
        </div>
        ...
        <div class="mb-2 d-flex align-items-center justify-content-between">
          ...
          <div class="d-flex align-items-center gap-2 small text-muted">
            <img v-if="currentTag && currentTag.src" :src="currentTag.src" class="sg-tag-preview" alt="선택 스티커" />
        ...
        <div v-if="alreadyGiven && !errorMsg" class="alert alert-info py-2 px-3 mt-2">
          이미 이 산책에 스티커를 보냈습니다.
        </div>
      </div>

      <div class="sg-footer">
        <button class="btn btn-secondary" @click="close" :disabled="sending">닫기</button>
      </div>
    </div>
  </div>
</template>
...
async function chooseAndSend(tag) {
  ...
  try {
    ...
    const dupHint = /duplicate|unique|ORA-00001|already/i.test(rawMsg) || status === 409
    const msg = dupHint ? "이미 스티커를 보냈습니다." : (rawMsg || "스티커 전송 실패")
    errorMsg.value = msg
    emit("error", msg)
  } finally {
    sending.value = false
...
<style scoped>
.sg-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,.5); display: grid; place-items: center; z-index: 2200; }
.sg-card { width: min(520px, 92vw); background: #fff; border-radius: .6rem; overflow: hidden; box-shadow: 0 12px 40px rgba(0,0,0,.25); position: relative; }
...
.sg-tags { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: .6rem; margin-top: .6rem; }
.sg-tag { border: 1px solid #ddd; border-radius: .75rem; background: #fff; padding: .6rem .5rem; display: grid; place-items: center; cursor: pointer; transition: transform .06s ease; }
.sg-tag:hover { transform: translateY(-1px); }
.sg-tag.active { border-color: #6f5034; box-shadow: 0 0 0 .2rem rgba(111,80,52,.15); }
.sg-tag:disabled { opacity: .6; cursor: default; }
