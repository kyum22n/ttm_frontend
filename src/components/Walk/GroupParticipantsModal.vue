  ...
  <div v-if="modelValue" class="gp-backdrop" @click.self="close">
    <div class="gp-card">
      ...
      <div class="gp-body">
        ...
        <!-- 승인 목록 -->
        <div v-else>
          <div v-if="approved.length === 0" class="text-center text-muted py-4">
            승인된 참가자가 없습니다.
          </div>
          <ul v-else class="list-group">
            <li
            ...
            >
              <div class="d-flex align-items-center gap-3">
                <img
                  class="gp-avatar"
                  :src="metaMap.get(a.userId)?.avatar || FALLBACK_AVATAR"
                  alt="상대 프로필"
                />
                <div class="gp-texts">
                  <div class="gp-title">
...
const authorIdFromStore = computed(() =>
  Number(store.state?.post?.detail?.postUserId || 0)
);
  ...
  await Promise.all(
    [...userIds].map(async (uid) => {
      if (!updates.has(uid))
        updates.set(
          uid,
          (await fetchUserMeta(uid)) || { petName: "", avatar: "" }
        );
    })
  );
...
async function load() {
  ...
  hint.value = "";
  if (!props.postId) {
    hint.value = "postId가 없습니다.";
    return;
  }
  if (!myUserId.value) {
    hint.value = "로그인 정보가 없습니다.";
    return;
  }
...
function approve(p) {
  if (canModerate.value) postStatus("A", p);
}
function reject(p) {
  if (canModerate.value) postStatus("R", p);
}
