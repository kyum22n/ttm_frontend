    ...
    <div class="modal-card">
      ...
      <div class="modal-body">
        ...
        <div v-else-if="tab === 'received'">
          <div v-if="received.length === 0" class="text-center text-muted py-4">
            받은 신청이 없습니다.
          </div>
                ...
                <div class="room-texts">
                  <div class="room-title">
                    ...
                    <span
                      class="status"
                      :class="
                        (w.rstatus || 'P').toUpperCase() === 'A'
                          ? 'ok'
                          : (w.rstatus || 'P').toUpperCase() === 'R'
                          ? 'rej'
                          : 'pend'
                      "
              ...
              <div class="d-flex align-items-center gap-2">
                <template v-if="(w.rstatus || 'P').toUpperCase() === 'P'">
                  <button
                    ...
                    @click="accept(w)"
                    :disabled="actingId === w.requestOneId || loading"
                  >
                    수락
                  </button>
                  <button
                ...
                <!-- 채팅 버튼 (상대 = 신청자) -->
                <WalkChatButton
                  :target-user-id="w.requestUserId"
                  label="채팅"
                />
              </div>
            </li>
        ...
        <div v-else-if="tab === 'sent'">
          <div v-if="sent.length === 0" class="text-center text-muted py-4">
            내가 신청한 내역이 없습니다.
          </div>
          <ul v-else class="list-group">
            <li
            ...
            >
              ...
              <div class="d-flex align-items-center gap-2">
                <WalkChatButton
                  :target-user-id="w.receiveUserId"
                  label="채팅"
                />
              </div>
            </li>
        ...
        <!-- 그룹 신청 -->
        <div v-else>
          <div v-if="groups.length === 0" class="text-center text-muted py-4">
            그룹 산책 신청이 없습니다.
          </div>
          <ul v-else class="list-group">
            <li
            ...
            >
              <div class="d-flex align-items-center gap-3">
                <img
                  class="avatar"
                  :src="
                    metaMap.get(ownerMap.get(g.postId))?.avatar ||
                    FALLBACK_AVATAR
                  "
                  alt="작성자 프로필"
                />
                <div class="room-texts">
                  <div class="room-title">
                    ...
                    <template v-else>
                      <span
                        class="status"
                        :class="partBadgeClass(g.applyStatus)"
                      >
...
async function enrichMetaForLists() {
  const oppReceived = [
    ...new Set(received.value.map((w) => w.requestUserId).filter(Boolean)),
  ];
  const oppSent = [
    ...new Set(sent.value.map((w) => w.receiveUserId).filter(Boolean)),
  ];
  const ownerIds = [
    ...new Set(Array.from(ownerMap.value.values()).filter(Boolean)),
  ];
  ...
  try {
    ...
    groups.value =
      grp.status === "fulfilled"
        ? grp.value?.data?.list || grp.value?.data || []
        : [];
