          ...
          <div class="room-texts">
            <div class="room-title">
              ...
              <span
                class="status"
                :class="
                  (rstatus || 'P').toUpperCase() === 'A'
                    ? 'ok'
                    : (rstatus || 'P').toUpperCase() === 'R'
                    ? 'rej'
                    : 'pend'
                "
                >{{ rstatusLabel }}</span
            ...
            <div class="room-sub small text-muted">
              ...
              <span
                v-if="applicantId"
                class="ms-1 badge"
                :class="isApplicantMe ? 'bg-primary' : 'bg-secondary'"
              >
            ...
            <div class="d-flex gap-2">
              ...
              <template v-else>
                <button
                  ...
                  :disabled="
                    loading || !currentReqId || rstatus !== 'A' || started
                  "
                  @click="mark(2)"
                >
...
const rstatusLabel = computed(
  () =>
    ({ P: "대기", A: "승인", R: "거절", C: "완료" }[
      (rstatus.value || "").toUpperCase()
    ] || "대기")
);

const me = computed(() => {
  try {
    return Number(JSON.parse(localStorage.getItem("user") || "{}").userId) || 0;
  } catch {
    return 0;
  }
});
const isApplicantMe = computed(
  () => applicantId.value != null && applicantId.value === me.value
);
...
function pick(it, keys) {
  for (const k of keys)
    if (it?.[k] != null && String(it[k]).length > 0) return it[k];
  return null;
}
function toMs(v) {
...
function chooseApproved(cands) {
  ...
  const live = cands.filter(
    (it) => !pick(it, ["walkEndedAt", "endedAt", "walkendedat", "ended_at"])
  );
...
async function loadStatus() {
  ...
  const reqList =
    req.status === "fulfilled" ? req.value?.data?.walkRequestList ?? [] : [];
  const recvList =
    recv.status === "fulfilled" ? recv.value?.data?.walkReceiveList ?? [] : [];

  let row = null;
  if (props.requestOneId) {
    row = [...reqList, ...recvList].find(
      (it) => (it.requestOneId ?? it.id) === props.requestOneId
    );
  }
  if (!row) {
    const asRequester = reqList.filter(
      (it) =>
        it.receiveUserId === props.partnerId &&
        (it.rstatus || "P").toUpperCase() === "A"
    );
    const asReceiver = recvList.filter(
      (it) =>
        it.requestUserId === props.partnerId &&
        (it.rstatus || "P").toUpperCase() === "A"
    );
  ...
  startedAt.value = pick(row, [
    "walkStartedAt",
    "startedAt",
    "walkstartedat",
    "started_at",
  ]);
  endedAt.value = pick(row, [
    "walkEndedAt",
    "endedAt",
    "walkendedat",
    "ended_at",
...
async function mark(code) {
  if (!currentReqId.value) {
    hint.value = "승인된 1:1 산책이 없습니다.";
    return;
  }
  if ((rstatus.value || "P").toUpperCase() !== "A") {
    hint.value = "승인된 산책만 시작/종료할 수 있습니다.";
    return;
  }
  if (!me.value) {
    hint.value = "로그인이 필요합니다.";
  ...
  if (code === 2 && started.value) {
    hint.value = "이미 산책이 시작되었습니다.";
    return;
  }
  if (code === 3 && (!started.value || ended.value)) {
    hint.value = "종료할 수 없는 상태입니다.";
    return;
  }
  ...
  try {
    ...
    errorMsg.value =
      e?.response?.data?.message || e?.message || "처리에 실패했습니다.";
  } finally {
    loading.value = false;
    nextCode.value = null;
  }
...
function onStickerError(msg) {
  errorMsg.value = msg || "스티커 전송 실패";
}

// 부모 콜백이 없을 때: reviewApi.createReview로 직접 호출 (requestOneId만 전송, postId 금지)
async function internalSendSticker({
  requestOneId,
  toUserId,
  reviewTagId,
  fromUserId,
