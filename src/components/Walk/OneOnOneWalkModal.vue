...
const props = defineProps({
  ...
  partnerId:    { type: Number,  required: true },   // 상대 프로필 userId
  partnerName:  { type: String,  default: "상대" },
  requestOneId: { type: Number,  default: null },    // 선택: 이미 아는 경우
})
...
const rstatus      = ref(null)     // 'P'|'A'|'R'|'C'
const applicantId  = ref(null)     // 신청자 userId
...
async function loadPartnerMeta() {
  ...
  try {
    ...
    if (pets.length > 0) {
      ...
      const main = pets[0]
      const petName = main?.petName || ""
      const avatar  = main?.petId ? `/pet/image/${main.petId}` : ""
...
function pick(it, keys){
  for (const k of keys) if (it?.[k] != null && String(it[k]).length > 0) return it[k]
  return null
}
...
/** 목록 2개 조회 → partnerId와 매칭되는 승인(A) 1건 확정 + 신청자/로그인자 관계 파악 */
async function loadStatus(){
  if (!me.value) { hint.value = "로그인이 필요합니다."; return }
  errorMsg.value = ""; hint.value = ""

  const [req, recv] = await Promise.allSettled([
    walkApi.getOneOnOneRequests(me.value),   // 내가 신청
    walkApi.getReceivedRequests(me.value),   // 내가 받은
  ])
...
async function mark(code){
  ...
  try{
    if (code === 2) {
      await walkApi.startWalk(currentReqId.value, me.value)
    } else {
      await walkApi.endWalk(currentReqId.value, me.value)
    }
    await loadStatus()   // 서버값으로 동기화
    emit("refreshed")
  } catch(e){
...
<style scoped>
/* 기존 스타일 유지 + WalkListModal의 아바타/텍스트 스타일 재사용 */
